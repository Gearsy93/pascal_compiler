...
namespace SyntaxAnalyzer
{
	public class Syntaxic
	{
		...
		{
			Console.WriteLine("EE Entered Accept_Loop_Operator_With_Postcondition");
			if (Accept_Raw("repeat"))
			{
				...
				Console.WriteLine("Done Accept_Loop_Operator_With_Postcondition");
				return true;
			}
			else return false;
		}
		...
		public bool Accept_Literal_Expression()
		{
			...
			if (!Accept_Literal_Summand()) Unhandled_Error("литерное слагаемое");
		...
		public bool Accept_Literal_Multiplier()
        {
			...
			if (Accept_Raw("("))
			{
				if (!Accept_Literal_Expression()) Unhandled_Error("литерное выражение");
