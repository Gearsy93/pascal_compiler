...
namespace SyntaxAnalyzer
{
	public class Syntaxic
	{
		...
		public bool Accept_Variables_Description()
        {
			...
			Tuple<bool, string> Bool_EType = Accept_Variable_Name();
			if (!Bool_EType.Item1)
			{
				return false;
			}
			...
			while (Accept_Raw(","))
			{
				Bool_EType = Accept_Variable_Name();
				if (!Bool_EType.Item1)
                {
        ...
        public bool Accept_Assignment_Operator()
        {
			...
			Tuple<bool, string> Id = Accept_Variable_Name();
			if (Id.Item1)
			{
				...
				if(Semantic_Analyzer.IsAssignable(Semantic_Analyzer.ConvertToType(Id.Item2), Semantic_Analyzer.ConvertToType(Expression.Item2)) == EType.NotDerivable)
                {
					Raise_Error(128);
                }
		...
		public Tuple<bool, string> Accept_Variable_Name()
		{
			Console.WriteLine("EE Entered Accept_Variable_Name");
			Tuple<bool, string> Bool_EType = Accept_Name();
			if (!Bool_EType.Item1)
			{
				return Bool_EType;
			}
			else
            {
				...
				Console.WriteLine("cock:" + Bool_EType.Item2);
				return Bool_EType;
			}
		}
		...
		public Tuple<bool, string> Accept_Real_Without_Sign()
		{
			...
			else
            {
				...
				return new Tuple<bool, string>(true, Last_Lexem.value);
			}
		}
		...
		public Tuple<bool, string> Accept_Name()
		{
