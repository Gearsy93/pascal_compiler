...
namespace LexicalAnalyzer
{
	...
	public class Lexical
	{
        ...
        {
			if (!(To_Accept.Contains(symbol + "") || System.Char.IsDigit(symbol) || System.Char.IsLetter(symbol)))
            {
				Raise_Error(6);
            }
        }
		...
		public class True_False : Lexem
		{
			new public string value;

			public True_False(int pos, int num, string input_value) : base(pos, num, input_value)
			{
				value = input_value;
			}
		}
		...
		public void PrintLexem()
        {
		    ...
		    while(!is_end)
            {
				...
				else if (CurrentLexem is True_False) CurrentLexem = (True_False)CurrentLexem;
				...
				else if (System.Char.IsLetter(symbol))
                {
					...
					while (raw.Length <= 9 && !found_key && !found_simple_type)
                    {
                        ...
                        else if (found_key)
                        {
                            ...
                            {
								CurrentLexem = new True_False(Reader.Line_Number, Reader.Line_Position, lowered);
							}
                            else
                            {
