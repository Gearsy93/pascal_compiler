...
namespace LexicalAnalyzer
{
	...
	public class Lexical
	{
		...
		public void Is_Acceptable(char symbol)
        {
			if (!(To_Accept.Contains(symbol + "") || System.Char.IsDigit(symbol) || System.Char.IsLetter(symbol) || symbol == '_'))
            {
				Raise_Error(6);
            }
        }
		...
		public class Real : Lexem
		{
			...
			{
				value = double.Parse(input_value, CultureInfo.InvariantCulture);
			}

		}

		public class Integer : Lexem
		{
			new public int value;

			public Integer(int pos, int num, string input_value) : base(pos, num, input_value)
			{
		...
		public class Boolean : Lexem
		{
			new public string value;

			public Boolean(int pos, int num, string input_value) : base(pos, num, input_value)
			{
				value = input_value;
			}
		}
		...
		public static string Get_Error(int code)
		{
			...
			switch (code)
			{
				...
				case 41: Error_Text = "Операция не применима к типам string и char"; break;
				...
				case 309: Error_Text = "Переменная не объявлена"; break;
				case 310: Error_Text = "Ошибка типа/ожидалось выражение"; break;
			}
			return Error_Text;
		}

		public string GetStringTypeLexel(Lexem Lexem)
        {
			return Lexem.GetType().ToString().ToLower().Substring(24);
		}
		...
		public void PrintLexem()
        {
		    ...
		    while(!is_end)
            {
				//Создание ссылки базового класса на объект наследуемого
				...
				else if (CurrentLexem is Integer) CurrentLexem = (Integer)CurrentLexem;
				...
				else if (CurrentLexem is Boolean) CurrentLexem = (Boolean)CurrentLexem;
		...
		//Проверка символа на отображаемость
		...
		public Lexem NextSym()
        {
			...
			if (Reader.Count < Reader.ProgramText.Length)
			{
				...
				string[] limiter = { ",", ".", "'", "(", ")", "[", "]", "{", "}", ":", ";" };
				string[] specifier = { "^", "#", "$" };
				string[] keywords_ext = { "if", "do", "of", "or", "in", "to", "end", "var", "div", "and", "not", "for", "mod", "nil", "set", "then", "else", "case", "file", "goto", "type", "with", "begin", "while", "downto", "packed", "record", "repeat", "until", "program", "function", "procedure", "true", "false"};
				...
				if(System.Char.IsDigit(symbol))
				{
					...
					else
					{
						...
						CurrentLexem = new Integer(Reader.Line_Number, Reader.Line_Position, raw);
						Reader.Back();
					}
				}
				else if (System.Char.IsLetter(symbol))
                {
					...
					while (raw.Length <= 9 && !found_key && !found_simple_type)
                    {
						...
						foreach (string keyword in keywords_ext)
                        {
							if (lowered == keyword)
                            {
                        ...
                        {
							//Находятся в списке ключевых слов
                            ...
                            {
								CurrentLexem = new Boolean(Reader.Line_Number, Reader.Line_Position, lowered);
							}
                            else
                            {
				...
				else if (limiter.Contains("" + symbol))
				{
					...
					else if (symbol == '(')
					{
                        ...
                        {
							Reader.Back();
							CurrentLexem = new Limiter(Reader.Line_Number, Reader.Line_Position, st);
						}
					}
					else
                    {
			...
			//Комментарии пропускаем, синтаксический не работает с ними, возвращаем следующую лексему
